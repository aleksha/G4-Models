...
//------------------------------------------------------------------------------
RFZDetectorConstruction::RFZDetectorConstruction()
...
RFZDetectorConstruction::~RFZDetectorConstruction(){ }
//------------------------------------------------------------------------------
G4VPhysicalVolume* RFZDetectorConstruction::Construct()
{
...
// Materials
  ...
  G4Element* H  = new G4Element("Hydrogen", "H",  z=1, a= 1.08*g/mole);
  ...
  G4Material* c_mat = new G4Material(name="c_mat"  , density = 32.2354*kg/m3, ncomponents=2);

  c_mat->AddMaterial(  ArGas,  fractionmass = 0.978811);
  c_mat->AddMaterial( CH4Gas,  fractionmass = 0.021189);
  ...
  G4Material* a_mat = nist->FindOrBuildMaterial("G4_Al");
  G4Material* m_mat = nist->FindOrBuildMaterial("G4_MYLAR" );
  ...
  G4double hyd_layer = 400.00*mm;
  G4double alu_layer =   0.05*mm;
  G4double myl_layer =   0.05*mm;
  G4double gap_layer =  50.00*mm;
  G4double csc_layer =   6.00*mm; // distance btw cathode planes


  G4double dum_layer = 0.001*mm;

  G4double l00_z = hyd_layer;
  G4double l01_z = alu_layer;
  G4double l02_z = hyd_layer;
  G4double l03_z = myl_layer;
  G4double l04_z = gap_layer;
  G4double l05_z = csc_layer;
  ...
  G4LogicalVolume* logicLV01 = new G4LogicalVolume(solidLV01, a_mat , "LV01");
  G4LogicalVolume* logicLV02 = new G4LogicalVolume(solidLV02, h_mat , "LV02");
  G4LogicalVolume* logicLV03 = new G4LogicalVolume(solidLV03, m_mat , "LV03");
  G4LogicalVolume* logicLV04 = new G4LogicalVolume(solidLV04, c_mat , "LV04");
  G4LogicalVolume* logicLV05 = new G4LogicalVolume(solidLV05, c_mat , "LV05");
  ...
  G4ThreeVector l01_pos; l01_pos.set(0,0, 0.5*alu_layer + 1.0*hyd_layer                                                 );
  G4ThreeVector l02_pos; l02_pos.set(0,0, 1.0*alu_layer + 1.5*hyd_layer                                                 );
  G4ThreeVector l03_pos; l03_pos.set(0,0, 1.0*alu_layer + 2.0*hyd_layer + 0.5*myl_layer                                 );
  G4ThreeVector l04_pos; l04_pos.set(0,0, 1.0*alu_layer + 2.0*hyd_layer + 1.0*myl_layer + 0.5*gap_layer                 );
  G4ThreeVector l05_pos; l05_pos.set(0,0, 1.0*alu_layer + 2.0*hyd_layer + 1.0*myl_layer + 1.0*gap_layer + 0.5*csc_layer );
