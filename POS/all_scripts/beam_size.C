...
int get_pad2(double x, double y){
    ...
    if(x>=0 && l>290 && l<=330) return 16;

   return -1;

}

void beam_size(double Position = 250., int Evts=MY_EVTS){
...
//    std::ifstream fNOI("./noise.data" , std::ios::in);
    std::ifstream fNOI("/home/user/Data/Noise/MERGED/noise.data" , std::ios::in);
//    std::ifstream fNOI("/home/user/Data/Noise/ohio/cpu0/noise_events.data" , std::ios::in);

    TString pFADC,pDIFF,pTEMP,pDEMP,pZ;
    ...
    for(int pad=0;pad<17;pad++){
        ...
        hZ[pad]    = new TH1F(pZ   ,";Z_{Ionization-Anode};Electrons",200,240,260);
    ...
    while( fOUT >> ev >> vol >> dE >> xi >> yi >> zi >> xf >> yf >> zf >> t ){
        if(ev>EVENT){
            ...
            EVENT=ev;
//            if( !(EVENT%1000) ) std::cout << "Event : " << EVENT << "\n";
        }
    ...
    for(int bin=1;bin<2551;bin++){
        std::cout << hFADC[2]->GetBinContent(bin) << "\n";
    }
    ...
    while( fNOI >> Val ){
        nbin++;
        if(nbin<2551){
            hFADC[2]->SetBinContent( nbin, hFADC[2]->GetBinContent(nbin) + Val );
            hDIFF[2]->SetBinContent( nbin, hDIFF[2]->GetBinContent(nbin) + Val );
        }
    ...
    double rms_b[16]; double pd[16];
    TCanvas* canvZ = new TCanvas("canvZ","canvZ",1000,1000);
    canvZ->Divide(4,4);
    for(int cn=1;cn<17;cn++){
        canvZ->cd(cn);
        gStyle->SetOptStat(0);
        hZ[cn]->SetLineWidth(2);
        hZ[cn]->Draw();
        pd[cn-1]=cn;
        rms_b[cn-1] = hZ[cn]->GetRMS();
        std::cout << "Pad" << cn << " : mean = " << hZ[cn]->GetMean() << "\t rms = " ;
        std::cout << hZ[cn]->GetRMS() << " +/-" << hZ[cn]->GetRMSError() << "\n";
    }
    canvZ->Print("DistoZ.png");
    canvZ->Close();

    TCanvas* canvG = new TCanvas("canvG","canvG",800,800);
    TGraph* grZ = new TGraph(16,pd,rms_b);
    grZ->SetTitle("RMS of beam vs pad ");
    grZ->GetXaxis()->SetTitle("Pad");
    grZ->GetYaxis()->SetTitle("RMS of beam, mm");
    grZ->SetMarkerStyle(24);
    grZ->SetLineWidth(2);
    gStyle->SetOptStat(0);
    grZ->Draw("APL");
    canvG->Print("RMS_beam.png");
    canvG->Close();
    ...
    for(int p=1;p<17;p++){
        pk_d[p-1]=peak_d[p];
        pk_n[p-1]=peak_n[p];
        dpk[p-1] =peak_n[p]-peak_d[p];
        if(p<10) std::cout << "Pad  " << p  << "\t"  << peak_n[p] << "\t" << peak_d[p] << "\t\t" << peak_n[p]-peak_d[p] << "\n";
        else     std::cout << "Pad "  << p  << "\t"  << peak_n[p] << "\t" << peak_d[p] << "\t\t" << peak_n[p]-peak_d[p] << "\n";
    }


    TCanvas* canvN = new TCanvas("canvN","canvN",800,800);
    TGraph* grN = new TGraph(16,pd,pk_n);
    TGraph* grD = new TGraph(16,pd,pk_d);
    TGraph* grS = new TGraph(16,pd,dpk);
    grN->SetTitle("Peak position");
    grD->SetTitle("Peak position");
    grS->SetTitle("Shift due to diffusion");

    grN->GetXaxis()->SetTitle("Pad");
    grD->GetXaxis()->SetTitle("Pad");
    grS->GetXaxis()->SetTitle("Pad");

    grN->GetYaxis()->SetTitle("Maximum position, ns");
    grD->GetYaxis()->SetTitle("Maximum position, ns");
    grS->GetYaxis()->SetTitle("Difference, ns");
    ...
    grD->Draw("APL");
    grN->Draw("same PL");
    canvN->Print("Peaks.png");
    grS->Draw("APL");
    canvN->Print("ShiftPos.png");
    canvN->Close();
