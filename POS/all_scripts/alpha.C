...
void alpha(double Position = 250., int Evts=1){
...
//==============================================================================
    ...
    std::ifstream fOUT("./calib.data" , std::ios::in);
            ...
            if(N_e>0){
                ...
                for(int ee=0;ee<N_e;ee++){
                    ...
                    for(int iii = 0 ; iii<125; iii++  ){
                        hTEMP[fpad]->Fill(t_anod + tt, Digi[iii]);
                        hDEMP[fpad]->Fill(d_anod + tt, Digi[iii]);
                        tt = tt + 40 ;
                    }
                }
            }
   ...
   std::cout << "TH1 : " << hFADC[0]->Integral() << "\n";
   double Integral = 0;
   for(int bin=1;bin<2551;bin++){
       Integral = Integral + hFADC[0]->GetBinContent(bin);
   }
   std::cout << "SUM : " << Integral << "\n";
