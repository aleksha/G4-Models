...
//------------------------------------------------------------------------------
CSCSteppingAction::~CSCSteppingAction(){  myOUT.close(); }
//------------------------------------------------------------------------------
...
//    double g_time =  0.5 * (g_pre_time + g_post_time);
    ...
    if( myOUT.is_open() && tr_ed>0. && (vol==7 || vol==9 || vol==11) )
       myOUT << ev_id << " " << tr_id  << " " << vol << " " << tr_ed << " " << p_code << G4endl;
  }
